...
const DRAWER_PRIMARY_NAV: DrawerNavItem[] = [
  { label: 'Dashboard', icon: Home, path: '/' },
  { label: 'Projects', icon: FolderKanban, path: '/projects' },
  { label: 'Clients', icon: Users, path: '/clients' }
];

const DRAWER_SALES_NAV: DrawerNavItem[] = [
  { label: 'Invoices', icon: Receipt, path: '/invoices' },
  { label: 'Quotations', icon: FileSignature, path: '/quotations' },
  { label: 'CSR', icon: ClipboardCheck, path: '/csr' },
  { label: 'Waybills', icon: Truck, path: '/waybills' }
];

const DRAWER_PRE_SALES_NAV: DrawerNavItem[] = [
  { label: 'RFQ', icon: ClipboardList, path: '/rfqs' },
  { label: 'BOQ', icon: FileText, path: '/boqs' }
];

const DRAWER_UTILITY_NAV: DrawerNavItem[] = [
  { label: 'Reports', icon: PieChart, path: '/reports' },
  { label: 'Compliance Hub', icon: CheckCircle2, path: '/compliance' },
  { label: 'Item Library', icon: FolderKanban, path: '/item-library' },
            ...
            <aside className="relative z-10 w-[280px] bg-[#FAF6EF] dark:bg-neutral-900 h-full p-5 flex flex-col justify-between border-r border-[#EADBB8] dark:border-neutral-800 shadow-2xl">
              <div>
                ...
                <nav className="mt-6 space-y-1.5">
                  {DRAWER_PRIMARY_NAV.map(({ label, icon: Icon, path }, index) => (
                    <button
                      key={path}
                      type="button"
                      onClick={() => handleNavigation(path)}
                      className={`flex w-full items-center justify-between px-3.5 py-2.5 rounded-xl font-semibold text-xs transition ${
                        index === 0
                          ? 'bg-[#E2BF7D]/20 text-[#8B6508] dark:text-amber-300 font-bold border border-[#B8860B]/30'
                          : 'text-slate-700 dark:text-neutral-300 hover:bg-[#F2E8D5] dark:hover:bg-neutral-800'
                      }`}
                    >
                      <span className="flex items-center gap-3">
                        <Icon className="w-4 h-4 text-amber-700/70" /> {label}
                      </span>
                ...
                {[
                  { label: 'Sales', items: DRAWER_SALES_NAV },
                  { label: 'Pre-Sales', items: DRAWER_PRE_SALES_NAV },
                  { label: 'Workspace', items: DRAWER_UTILITY_NAV }
                ].map(({ label, items }) => (
                  <div key={label} className="mt-5 border-t border-[#EADBB8] dark:border-neutral-800 pt-4">
                    <p className="px-3.5 mb-2 text-[9px] font-bold uppercase tracking-wider text-amber-800/70 dark:text-amber-400/80">
                      {label}
                    </p>
                    <div className="space-y-1.5">
                      {items.map(({ label: itemLabel, icon: Icon, path }) => (
                        <button
                          ...
                          type="button"
                          onClick={() => handleNavigation(path)}
                          className="flex w-full items-center justify-between px-3.5 py-2.5 rounded-xl text-slate-700 dark:text-neutral-300 hover:bg-[#F2E8D5] dark:hover:bg-neutral-800 font-semibold text-xs transition"
                        >
                          <span className="flex items-center gap-3">
                            <Icon className="w-4 h-4 text-amber-700/70" /> {itemLabel}
                          </span>
                          <ChevronRight className="w-3.5 h-3.5 text-amber-600" />
                        </button>
                      ))}
