...
| Files-tax-monthly-v1.md | ✅ Active | Engineering, Product | Monthly Files.tax compliance document — VAT position, WHT position, deadlines, attention items. Downstream of Technical-plan-v1.1.md (current technical baseline) |
...
| Block-A-accounting-foundation-core-spec-v1.md | ✅ Active | Engineering | Block A implementation specification — maps the Accounting Foundation Core to the closed Increments 1–6, names the two remaining gaps (reporting foundation, controlled reversal path), and fixes the downstream boundary. Downstream of Accounting-foundation-blueprint-v1.md and the CIT-Readiness Roadmap |
| Gap-1-journal-derived-reporting-foundation-spec-v1.md | ✅ Active | Engineering | Gap 1 implementation specification (v1.1) — read-only derivation of account balances, period totals, and trial balance from active posted journal lines (explicit reversal rule; reversed originals excluded, reversal entries active). Downstream of Block-A-accounting-foundation-core-spec-v1.md; prerequisite for the balance-authority switch and Block D |
...
### 2h. Block-A-accounting-foundation-core-spec-v1.md
**TL;DR:** The canonical Block A implementation specification. It maps each Block A capability (entity-scoped books, chart of accounts, posting kernel, source transaction boundary, periods, invariants, corrections, reporting foundation) to the closed Accounting Foundation Increments 1–6, classifies every component as EXISTING / CLOSED, EXISTING / RETAIN, REQUIRED GAP, VERIFY DURING IMPLEMENTATION, or DEFERRED / DOWNSTREAM, and determines that Block A is a substantially closed foundation gate, not a greenfield implementation block. Two required gaps remain: the journal-derived reporting foundation and a controlled posted-entry reversal path. It adds no new architecture and no statutory value.

### 2i. Gap-1-journal-derived-reporting-foundation-spec-v1.md
**TL;DR:** The canonical Gap 1 implementation specification (v1.1). Gap 1 derives account balances, period totals (with bounded opening/closing), and a trial balance from active posted journal lines through one read-only boundary. Reversal rule is explicit: a posted entry is active only if it has not been reversed by a posted reversal entry; a reversed original is excluded; the reversal entry stays active unless itself reversed; the pair nets to zero as equal-and-opposite journal facts; the correction event stays audit-visible; no partial-reversal support. Determination: the reporting foundation is genuinely missing — no balance derivation, trial balance, or reporting view exists. It is one small increment with the fixed boundary Journal → deterministic account/period aggregation → trial balance: derive live, no new table, no cache, no UI, no tax logic. It enables the balance-authority switch and Block D (P&L), and never mutates the journal.
...
- `Files-tax-monthly-v1.md` **depends on** `Technical-plan-v1.1.md` (the current technical baseline; `Technical-plan-v1.2.md` does not exist and was not created) and on the reconciliation evidence in `NRS-docs/OBLIGATION-LOOKUP-INDEX.md`. It reuses the existing notification, push, dashboard, and Compliance Hub infrastructure. Do not start its build order until the roadmap gates defined by the v1.1 baseline audit are closed.
...
- `Block-A-accounting-foundation-core-spec-v1.md` **implements the architecture of** `Accounting-foundation-blueprint-v1.md` for Block A and depends on the CIT-Readiness Roadmap for the block definition. It is the single canonical Block A specification. It does not replace the blueprint, does not create a second accounting architecture, and must be updated in place when a Block A gap closes. Do not create competing Block A documents.
- `Gap-1-journal-derived-reporting-foundation-spec-v1.md` **is the canonical specification for Gap 1 of** `Block-A-accounting-foundation-core-spec-v1.md` (section 16). It is a read-only derivation layer. It never mutates the journal, never embeds tax logic, and stays separate from Gap 2 (controlled reversal path). Update it in place when the gap closes; do not create competing reporting-foundation documents.
