...
| Gap-1-journal-derived-reporting-foundation-spec-v1.md | ✅ Active | Engineering | Gap 1 implementation specification — read-only derivation of account balances, period totals, and trial balance from posted, un-reversed journal lines. Downstream of Block-A-accounting-foundation-core-spec-v1.md; prerequisite for the balance-authority switch and Block D |
...
**TL;DR:** The canonical Gap 1 implementation specification. Gap 1 derives account balances, period totals (with opening/closing), and a trial balance from posted, un-reversed journal lines through one read-only boundary. Determination: the reporting foundation is genuinely missing — no balance derivation, trial balance, or reporting view exists. It is one small increment: derive live, no new table, no cache, no UI (UI is deferred to a later task under the Adaptive UI/UX authority), no tax logic. It enables the balance-authority switch and Block D (P&L), and never mutates the journal.
