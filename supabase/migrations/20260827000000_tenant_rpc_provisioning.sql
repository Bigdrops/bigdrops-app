-- ============================================================
-- TENANT RPC PROVISIONING + PUBLIC REGRESSION FIXES
-- ============================================================
-- Part 1: Re-create public.record_activity_event with the full
--         whitelist (letter entity + PAYMENT_VOIDED /
--         ATTACHMENT_UPLOADED / DUPLICATE events). The deployed
--         version (20260821000000) lost these entries.
-- Part 2: Re-create public.record_payment_transaction and fix the
--         p_schema_name -> v_schema bug in its EXECUTE format args.
-- Part 3: Create public._prov_install_tenant_rpcs(p_schema_name)
--         which installs the 27 audit / lifecycle RPCs into a
--         tenant schema. This fixes the production failure:
--         "Could not find the function entity_bigdrops-main_main.
--         save_invoice_with_items_transaction(...) in the schema cache".
-- Part 4: Re-create public.provision_entity to call
--         _prov_install_tenant_rpcs during provisioning (step 8.9).
-- Part 5: Backfill RPCs into all existing tenant schemas.
-- Final:  NOTIFY pgrst so PostgREST reloads its schema cache.

-- ============================================================
-- PART 1 — public.record_activity_event (full whitelist)
-- ============================================================
-- Canonical body from 20260710100000_record_letter_events.sql.
...
-- ============================================================
-- PART 2 — public.record_payment_transaction (v_schema bug fix)
-- ============================================================
-- Fix: EXECUTE format(...) used the undeclared identifier
-- `p_schema_name` instead of the declared variable `v_schema`.
...
-- ============================================================
-- PART 3 — public._prov_install_tenant_rpcs(p_schema_name)
-- ============================================================
-- Installs the 27 lifecycle / audit RPCs into the tenant schema.
-- Each body is embedded as a dollar-quoted literal and executed via
-- string replacement:
--   __SCHEMA__      -> quote_ident(p_schema_name)  (headers, types)
--   __SCHEMA_TEXT__ -> p_schema_name               (v_schema := ...)
-- __SCHEMA_TEXT__ is replaced first because it shares the prefix.
-- Bodies are written against the canonical schema-aware sources.
-- Public-qualified calls (has_entity_permission, record_activity_event,
-- record_audit_log, compute_jsonb_diff, _prov_* , _audit_resolve_*)
-- stay public and resolve through the installer search_path.
...
BEGIN
    ...
    --    Source: 20260814000000_fix_invoice_item_discount_null.sql
    v_body := $b1$
...
$b1$;
    ...
    --    Source: 20260809070000_invoice_composite_transactions.sql
    v_body := $b2$
...
$b2$;
    ...
    --    Source: 20260809070000_invoice_composite_transactions.sql (bug fixed)
    v_body := $b3$
...
$b3$;
    ...
    -- 4. record_invoice_created
    --    Source: 20260809040000_invoice_audit_schema_aware.sql
    v_body := $b4$
...
$b4$;
    ...
    -- 5. record_invoice_status_changed
    --    Source: 20260809040000_invoice_audit_schema_aware.sql
    v_body := $b5$
...
$b6$;
    ...
    -- 7. record_payment_attachment_uploaded
    --    Source: 20260809040000_invoice_audit_schema_aware.sql
    v_body := $b7$
...
$b7$;
    ...
    -- 8. record_waybill_created
    --    Source: 20260821000000_waybill_csr_audit_schema_aware.sql
    v_body := $b8$
...
$b8$;
    ...
    -- 9. record_waybill_status_changed
    --    Source: 20260821000000_waybill_csr_audit_schema_aware.sql
    v_body := $b9$
...
$b13$;
    ...
    -- 14. record_quotation_status_changed
    --    Source: 20260520090002_quotations.sql
    v_body := $b14$
...
$b17$;
    ...
    -- 18. record_project_document_added
    --    Source: 20260520090001_projects.sql
    v_body := $b18$
...
$b18$;
    ...
    -- 19. record_project_linked_activity
    --    Source: 20260520090001_projects.sql
    v_body := $b19$
...
$b19$;
    ...
    -- 20. record_letter_created
    --    Source: 20260710100000_record_letter_events.sql
    v_body := $b20$
...
$b21$;
    ...
    -- 22. record_letter_status_changed
    --    Source: 20260710100000_record_letter_events.sql
    v_body := $b22$
...
$b24$;
    ...
    --    audit_logs stays in public; the tenant schema holds the RPC so
    --    tenantClient.rpc('record_audit_log', ...) resolves schema-scoped.
    v_body := $b25$
...
$b25$;
    ...
    -- 26. record_activity_event
    --    Source: 20260710100000_record_letter_events.sql
    --    activity_events stays in public; the tenant schema holds the RPC so
    --    tenantClient.rpc('record_activity_event', ...) resolves schema-scoped.
    v_body := $b26$
...
$b26$;
    ...
    -- 27. revert_invoice_to_quotation_transaction
    --    Source: 20260820000000_fix_revert_quotation_status_mapping.sql
    --    quotations/quotation_items stay in public; invoice deletion is
    --    executed against the tenant schema via v_schema.
    v_body := $b27$
...
-- ============================================================
-- PART 4 — provision_entity(): add step 8.9 (install tenant RPCs)
-- ============================================================
-- Re-creates the provisioning orchestration entry point so new
-- entities receive the 27 lifecycle / audit RPCs during creation.
...
-- ============================================================
-- PART 5 — Backfill RPCs into existing tenant schemas
-- ============================================================
-- Entities provisioned before this migration exist with zero
-- lifecycle RPCs. Install the 27 RPCs into every entity_* schema.
