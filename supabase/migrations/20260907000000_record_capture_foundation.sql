...
DO $block$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'tax_input_entries' AND relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
  ...
  ELSE
    RAISE NOTICE 'tax_input_entries not found — skipping extend';
  END IF;
END $block$;
...
DO $block$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'activity_events' AND relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
...
$function$;
  ELSE
    RAISE NOTICE 'activity_events table not found — skipping record_activity_event';
  END IF;
END $block$;

-- 2b. __SCHEMA__.record_activity_event (tenant-scoped RPC)
-- ponytail: __SCHEMA__ is a provisioning template placeholder — skip on direct push

-- 2c. record_expense_recorded RPC (domain event)

DO $block$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'record_activity_event' AND pronamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')) THEN
...
$function$;
  ELSE
    RAISE NOTICE 'record_activity_event not found — skipping record_expense_recorded';
  END IF;
END $block$;

-- Tenant-scoped version: skipped (placeholder __SCHEMA__)
