...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'authenticated_users_can_read_activity_events' AND tablename = 'activity_events') THEN
    CREATE POLICY authenticated_users_can_read_activity_events ON activity_events FOR SELECT TO authenticated USING (true);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'team_members_can_view_activity_events' AND tablename = 'activity_events') THEN
    CREATE POLICY team_members_can_view_activity_events ON activity_events FOR SELECT TO authenticated USING (true);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'team_members_can_view_all_audit_logs' AND tablename = 'audit_logs') THEN
