...
BEGIN
    ...
    IF to_regclass(v_schema || '.invoices') IS NULL THEN
        RAISE NOTICE 'Schema % does not exist — skipping invoice aggregate data migration (non-production environment)', v_schema;
        RETURN;
    END IF;
