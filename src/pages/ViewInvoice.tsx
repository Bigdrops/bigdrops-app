...
} from "@/domain/invoice/previewModel";
...
export default function ViewInvoice() {
  ...
  const [pdfOutput, setPdfOutput] = useState<PdfOutputSettingsValue>(
    DEFAULT_INVOICE_PDF_OUTPUT,
  );
  const [projectLinkOpen, setProjectLinkOpen] = useState(false);
  const [reverting, setReverting] = useState(false);
  const [advanceSheetMode, setAdvanceSheetMode] =
    useState<AdvanceSheetMode>("create");
  const [selectedAdvanceInvoice, setSelectedAdvanceInvoice] = useState<
    any | null
  >(null);
  const [advanceSaving, setAdvanceSaving] = useState(false);
  const [advancePdfGenerating, setAdvancePdfGenerating] = useState(false);
  const [advanceDeleteConfirmOpen, setAdvanceDeleteConfirmOpen] =
    useState(false);
  ...
  const [advanceSuffixValue, setAdvanceSuffixValue] = useState(
    ADVANCE_SUFFIX_DEFAULT,
  );
  const [advancePrimaryLabel, setAdvancePrimaryLabel] = useState(
    ADVANCE_PRIMARY_LABEL_DEFAULT,
  );
  const [advanceSecondaryLabel, setAdvanceSecondaryLabel] = useState(
    ADVANCE_SECONDARY_LABEL_DEFAULT,
  );
  const [pendingVoidPaymentId, setPendingVoidPaymentId] = useState<
    string | null
  >(null);
  ...
  const activeAdvanceRecord = useMemo(() => {
    ...
    const fallbackConfig = parseCustomFields(
      primaryRelatedAdvanceInvoice?.custom_fields,
    );
    ...
    return {
      ...
      id:
        advanceMetadata.legacy_child_invoice_id ||
        primaryRelatedAdvanceInvoice?.id ||
        null,
      invoice_number:
        ...
        "Advance Invoice",
      invoice_title:
        primaryRelatedAdvanceInvoice?.invoice_title ||
        invoice?.invoice_title ||
        "Advance Invoice",
      total: Number(
        advanceMetadata.amount || primaryRelatedAdvanceInvoice?.total || 0,
      ),
      status:
        advanceMetadata.status ||
        primaryRelatedAdvanceInvoice?.status ||
        "unpaid",
      issue_date:
        advanceMetadata.issued_at ||
        primaryRelatedAdvanceInvoice?.issue_date ||
        invoice?.issue_date ||
        null,
      due_date:
        advanceMetadata.due_at ||
        primaryRelatedAdvanceInvoice?.due_date ||
        invoice?.due_date ||
        null,
      amount_in_words:
        primaryRelatedAdvanceInvoice?.amount_in_words ||
  ...
  const visibleAdvanceInvoices =
    hasParentAdvanceConfig && activeAdvanceRecord ? [activeAdvanceRecord] : [];
  ...
  const resolvedSignatory = useMemo(
    () =>
      resolveDocumentSignatory(
        (customFields as any)?.signatoryId,
  ...
  const previewTotalsSource = useMemo(() => {
    ...
    const savedColumns = Array.isArray(customFields?.columnConfig)
    ...
    return {
      ...
      balanceDue: Math.max(
        0,
        (totals.totalPayable || 0) - Number(viewModel.settledTotal || 0),
      ),
    };
  ...
  const openAdvanceDetails = useCallback(
    (advanceInvoice: any, mode: AdvanceSheetMode = "view") => {
  ...
  const closeAdvanceSheet = useCallback(
    (nextOpen: boolean) => {
      if (nextOpen) {
        ui.openSheet(SHEET_ADVANCE);
        return;
  ...
  const openSavedAdvanceActions = useCallback(
    (advanceInvoice: any | null | undefined) => {
      if (advanceInvoice) {
  ...
  const downloadInvoicePdfDocument = useCallback(
    async ({
      targetInvoice,
      targetItems,
      targetPayments,
      ...
      const targetCustomFields = parseCustomFields(
        targetInvoice?.custom_fields,
      );
      const targetTemplateId: InvoicePdfTemplateId =
        normalizeInvoicePdfTemplateId(targetCustomFields?.pdfTemplateId) ||
        pdfTemplateId;
      const savedColumns = Array.isArray(targetCustomFields?.columnConfig)
        ? targetCustomFields.columnConfig
        : BUILTIN_COLUMNS;
      const totals = computeDocument({
      ...
      const settledTotal = (
        Array.isArray(targetPayments) ? targetPayments : []
      ).reduce((sum, payment) => {
      ...
      const targetPreviewModel = buildInvoicePreviewModel({
        ...
        pdfOutput,
        signatory: resolveDocumentSignatory(
          targetCustomFields?.signatoryId,
          Array.isArray(signatories) ? signatories : [],
        ),
        ...
        balanceDue: Math.max(
          0,
          (totals.totalPayable || Number(targetInvoice?.total || 0)) -
            settledTotal,
        ),
        totals: {
      ...
      await generateInvoicePdf({
        model: {
          ...
          issuer: {
            ...
            addressLines: Array.isArray(targetPreviewModel?.companyPreviewLines)
              ? targetPreviewModel.companyPreviewLines
              : [],
          ...
          recipient: {
            ...
            addressLines: Array.isArray(targetPreviewModel?.clientPreviewLines)
              ? targetPreviewModel.clientPreviewLines
              : [],
            phone: String(client?.phone || ""),
            email: String(client?.email || ""),
          },
          headerFields: Array.isArray(targetPreviewModel?.previewDetailRows)
            ? targetPreviewModel.previewDetailRows
            : [],
          columns: resolvedTable.columns,
          mergeQtyUnit: resolvedTable.mergeQtyUnit,
          items: (Array.isArray(targetItems) ? targetItems : []).map(
            (item, index) => ({
              id: String(item.id || item._uiKey || index),
              rowType:
                item.row_type === "group_header" ? "group_header" : "line",
              ...
              customData: {
                ...
                ...(item.row_type === "group_header"
                  ? {
                      showSubtotal:
                        targetCustomFields?.groupMeta?.[item.group_id]
                          ?.showSubtotal === true,
                    }
                  : {}),
              },
          ...
          totals: {
            ...
            rows: (Array.isArray(targetPreviewModel?.previewTotals)
              ? targetPreviewModel.previewTotals
              : []
            ).map((row) => ({
              ...
              amount:
                Number(String(row.value || "0").replace(/[^\d.-]/g, "")) || 0,
              emphasis: row.emphasis === true,
            })),
            amountInWords: String(
              targetPreviewModel?.previewAmountInWords || "",
            ),
            ...
            advanceSummary: targetPreviewModel?.advanceSummary
              ? {
                  ...
                  primaryLabel:
                    targetAdvanceSummary?.primaryLabelWithPercent ||
                    targetPreviewModel.advanceSummary.primaryLabel,
                  secondaryLabel:
                    targetAdvanceSummary?.secondaryLabelWithPercent ||
                    targetPreviewModel.advanceSummary.secondaryLabel,
                }
              : null,
          },
          bankDetails: pdfOutput.showBankDetails
            ? targetPreviewModel?.selectedPreviewBank
            : null,
          notes: targetInvoice.notes
            ? {
                title: String(targetCustomFields?.notesTitle || "Notes"),
                content: targetInvoice.notes,
                format: "html",
              }
            : null,
          terms: targetInvoice.terms
            ? {
                title: String(
                  targetCustomFields?.termsTitle || "Terms and Conditions",
                ),
                content: targetInvoice.terms,
                format: "html",
              }
            : null,
          additionalSections: [],
          ...
          footerText: pdfOutput.showFooter
            ? String(settingsData?.footer_text || "")
            : "",
          tagline: pdfOutput.showTagline
            ? String(settingsData?.company_tagline || "")
            : "",
  ...
  const handleSaveCustomization = useCallback(
    async (
      ...
      try {
        ...
        showToast(
          "Settings saved",
          "Invoice PDF output settings updated.",
          "success",
        );
      } catch (err) {
        ...
        showToast(
          ...
          err instanceof Error
            ? err.message
            : "Could not save customize settings",
        );
      }
    },
  ...
  const handleToggleMergeQtyUnit = useCallback(async () => {
    ...
    try {
      ...
      showToast(
        ...
        nextCustomFields.mergeQtyUnit
          ? "Qty + Unit merge is now on for exports and preview."
          : "Qty + Unit merge is now off for exports and preview.",
        "success",
      );
    } catch (err) {
      showToast(
        ...
        err instanceof Error
          ? err.message
          : "Could not update the table export setting.",
      );
    }
  ...
  const handleAdvanceSave = async () => {
    ...
    try {
      ...
      showToast(
        "Advance invoice failed",
        error instanceof Error
          ? error.message
          : "Could not save advance invoice",
      );
    } finally {
  ...
  const handleAdvanceDownload = useCallback(async () => {
    ...
    try {
      ...
      showToast(
        ...
        error instanceof Error
          ? error.message
          : "Could not generate the advance invoice PDF.",
      );
    } finally {
  ...
  const handleDuplicate = async () => {
    ...
    try {
      navigate("/invoices/new", {
        state: await duplicateInvoiceDraft({
          invoice,
          items: Array.isArray(items) ? items : [],
        }),
      });
  ...
  const handleRevertToQuotation = async () => {
    ...
    setReverting(true);
    try {
      const createdQuotation = await revertInvoiceToQuotation({
        invoice,
        items: Array.isArray(items) ? items : [],
        customFields,
      });
  ...
  const confirmVoidPayment = async (reason: string) => {
    ...
    try {
      ...
      showToast(
        "Payment voided",
        "The payment has been reversed and invoice status updated.",
        "success",
      );
      ui.closeModal();
    } catch (error) {
      showToast(
        "Void failed",
        error instanceof Error ? error.message : "Could not void this payment.",
      );
    } finally {
      setVoiding(false);
  ...
  const relatedDocuments = [
      ...
      ? [
          {
            ...
            onClick: sourceDocument.id
              ? () =>
                  navigate(
                    `/${sourceDocument.type === "quotation" ? "quotations" : "invoices"}/${sourceDocument.id}`,
                  )
              : undefined,
          },
        ]
    ...
    ...(Array.isArray(relatedWaybills) ? relatedWaybills : []).map(
      (waybill: any) => ({
  ...
  const attachments = Array.isArray(customFields?.attachments)
    ? customFields.attachments.map((entry: any, index: number) => ({
        id: `attachment-${index}`,
        label: String(
          entry.label || entry.name || entry.url || `Reference ${index + 1}`,
        ),
      }))
    : [];
    ...
    <>
        ...
        actionRow={
          <>
            <button
              ...
              onClick={() => ui.openSheet(SHEET_RECORD_PAYMENT)}
      ...
      >
        <InvoiceViewPage
          ...
          paymentHistory={(Array.isArray(viewModel.paymentHistory)
            ...
            (advance: any, index: number) => {
              ...
              return {
                ...
                title:
                  ...
                  advance.invoice_title ||
                  "Advance Invoice",
                subtitle:
                  advConfig.primary_label ||
                  advConfig.primaryLabel ||
                  ADVANCE_PRIMARY_LABEL_DEFAULT,
