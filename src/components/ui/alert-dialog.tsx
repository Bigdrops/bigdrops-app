...
function AlertDialogOverlay({
  ...
  return (
    <AlertDialogPrimitive.Overlay
      ...
      className={cn(
        "fixed inset-0 z-50 bg-black/50 dark:bg-black/70 transition-opacity duration-200 data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
        className
      )}
      {...props}
    />
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Content
        ...
        data-size={size}
        className={cn(
          "group/alert-dialog-content fixed inset-0 z-50 flex items-center justify-center p-4 duration-100 outline-none data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
          /* content box — centered by parent flex, not translate */
          "pointer-events-auto w-full max-h-[var(--bd-overlay-dialog-max-height,100dvh)] overflow-y-auto overscroll-contain gap-3 rounded-xl bg-background p-4 ring-1 ring-foreground/10 data-[size=default]:max-w-xs data-[size=sm]:max-w-64 data-[size=default]:sm:max-w-sm data-open:zoom-in-95 data-closed:zoom-out-95",
          className
        )}
        {...props}
      />
