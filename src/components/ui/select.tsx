...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "flex w-full items-center justify-between gap-1.5 rounded-[var(--bd-radius-md)] border border-bd-border bg-bd-surface px-3 py-2 text-sm text-bd-text whitespace-nowrap transition-colors outline-none focus-visible:border-bd-button-primary-bg focus-visible:ring-2 focus-visible:ring-bd-button-primary-bg/15 disabled:cursor-not-allowed disabled:border-bd-border disabled:bg-bd-surface-muted disabled:text-bd-text-muted disabled:opacity-100 aria-invalid:border-bd-input-error aria-invalid:ring-2 aria-invalid:ring-destructive/20 data-placeholder:text-bd-text-muted data-[size=default]:h-10 data-[size=sm]:h-8 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 dark:aria-invalid:border-bd-input-error/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
        className
      )}
      {...props}
    >
