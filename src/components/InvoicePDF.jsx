...
const styles = StyleSheet.create({
  ...
  // Advance badge (shown under docNumber for advance invoices)
  advanceBadge: { backgroundColor: '#F59E0B', paddingVertical: 3, paddingHorizontal: 10, borderRadius: 20, marginTop: 4, alignSelf: 'flex-end' },
  advanceBadgeText: { color: 'white', fontSize: 8, fontFamily: 'Helvetica-Bold', textTransform: 'uppercase', letterSpacing: 1 },
  ...
  // Advance-mode totals
  projectValueRow: { flexDirection: 'row', justifyContent: 'space-between', borderTopWidth: 2, borderTopColor: '#1a1a1a', paddingTop: 6, marginTop: 4, marginBottom: 6 },
  projectValueLabel: { fontSize: 10, fontFamily: 'Helvetica-Bold', color: '#555' },
  projectValueAmount: { fontSize: 12, fontFamily: 'Helvetica-Bold', color: '#1a1a1a' },
  advanceDueBox: { backgroundColor: '#FEF3C7', borderWidth: 2, borderColor: '#F59E0B', borderRadius: 6, paddingVertical: 8, paddingHorizontal: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 },
  advanceDueLabel: { fontSize: 9, fontFamily: 'Helvetica-Bold', color: '#92400E' },
  advanceDueAmount: { fontSize: 14, fontFamily: 'Helvetica-Bold', color: '#B45309' },

  // Balance note (advance mode footer)
  balanceNote: { backgroundColor: '#EFF6FF', borderLeftWidth: 3, borderLeftColor: '#3B82F6', padding: 8, marginBottom: 12 },
  balanceNoteText: { fontSize: 8, color: '#1D4ED8', fontFamily: 'Helvetica-Bold' },
...
export default function InvoicePDF({ invoice, items = [], client, settings = {} }) {
  ...
  // ── Advance mode fields ───────────────────────────────────────────────────
  const isAdvanceMode    = invoice.thread_role === 'advance'
  const contractTotal    = Number(invoice.total_contract_value || 0)
  const advanceValue     = Number(invoice.advance_value || 0)
  const advanceMode      = invoice.advance_mode || 'percent'
  const balanceRemaining = Math.max(0, contractTotal - totalPayable)
  const advancePct       = advanceMode === 'percent' && advanceValue
    ? advanceValue
    : contractTotal > 0 ? Math.round((totalPayable / contractTotal) * 100) : 0
  const balancePct       = Math.max(0, 100 - advancePct)

  const advanceBadgeLabel = advanceValue
    ? (advanceMode === 'percent' ? `${advanceValue}% Advance Payment` : 'Advance Payment')
    : 'Advance Payment'
  ...
  // ── Build render rows with group subtotals ────────────────────────────────
  const renderRows = (() => {
    const rows = []
    ...
    items.forEach((item, index) => {
      if (item.row_type === 'group_header') {
        ...
        // Only accumulate when inside a named group — ungrouped items must not bleed into subtotal
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.docBlock}>
            ...
            {/* Advance badge — only on advance invoices */}
            {isAdvanceMode && (
              <View style={styles.advanceBadge}>
                <Text style={styles.advanceBadgeText}>{advanceBadgeLabel}</Text>
              </View>
            )}
          </View>
        </View>
        ...
        <View style={styles.twoCol}>
          ...
          <View style={styles.col}>
            ...
            {invoice.job_title
              ? <Text style={[styles.clientDetail, { color: '#1D4ED8', fontFamily: 'Helvetica-Bold' }]}>Job: {invoice.job_title}</Text>
              : null
            }
              ...
              <View key={'item_' + ri} style={rowStyle} wrap={false}>
                ...
                <View style={[styles.cDesc, { alignSelf: 'flex-start' }]}>
                  ...
                  {showItemImages && item.image_url
                    ? <Link src={item.image_url}><Image src={item.image_url} style={styles.itemThumb} /></Link>
                    : null
                  }
                </View>
                {mergeQtyUnit
        ...
        <View style={[styles.totalsSection, { marginTop: 10 }]} wrap={false}>
          <View style={styles.totalsBox}>
            ...
            {/* ── Advance mode: different totals presentation ── */}
            {isAdvanceMode && contractTotal > 0 ? (
              <>
                <View style={styles.projectValueRow}>
                  <Text style={styles.projectValueLabel}>Total Project Value</Text>
                  <Text style={styles.projectValueAmount}>NGN {contractTotal.toLocaleString()}</Text>
                </View>
                <View style={styles.advanceDueBox}>
                  <Text style={styles.advanceDueLabel}>
                    {advancePct > 0 ? `${advancePct}% ` : ''}Advance Payment Due
                  </Text>
                  <Text style={styles.advanceDueAmount}>NGN {totalPayable.toLocaleString()}</Text>
                </View>
              </>
            ) : (
              <>
        ...
        {/* ── Balance note — advance mode only ── */}
        {isAdvanceMode && contractTotal > 0 && balanceRemaining > 0 && (
          <View style={styles.balanceNote}>
            <Text style={styles.balanceNoteText}>
              {`Balance of ${balancePct > 0 ? balancePct + '% ' : ''}(NGN ${balanceRemaining.toLocaleString()}) due upon project completion.`}
            </Text>
          </View>
        )}
        ...
        {isAdvanceMode && contractTotal > 0 ? (
          <View style={styles.footer}>
            <Text style={styles.footerText}>
              ...
              {`Balance of NGN ${balanceRemaining.toLocaleString()} due upon project completion.`}
            </Text>
          </View>
        ) : footerText ? (
          <View style={styles.footer}>
