...
export default function ObsidianReceipt({ data }: { data: CommercialDocumentData }) {
  const safeData = data ?? ({} as CommercialDocumentData);
  const {
    title = '',
    customTitle = null,
    documentNumber = '',
  ...
  const safeDesign = (design || {}) as NonNullable<CommercialDocumentData['design']>;
