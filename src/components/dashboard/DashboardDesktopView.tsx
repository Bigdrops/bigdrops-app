...
type DashboardDesktopViewProps = {
  ...
  onQuickTileClick: (path: string) => void
  onRecentDocSelect: (doc: RecentDoc) => void
  onReportClick: () => void
  onOpenQuickAccess: () => void
  onQuickAccessOpenChange: (open: boolean) => void
  onProjectSelect: (project: RecentProject) => void
}

const naira = (amount: number) => formatNaira(amount, { round: true })

const typeStyle = {
  Invoice: { badge: 'bg-blue-50 text-blue-700 border-blue-200', dot: 'bg-blue-600', icon: FileText },
  Quotation: { badge: 'bg-violet-50 text-violet-700 border-violet-200', dot: 'bg-violet-600', icon: FileSignature },
  CSR: { badge: 'bg-orange-50 text-orange-700 border-orange-200', dot: 'bg-orange-600', icon: ClipboardCheck },
  Waybill: { badge: 'bg-slate-50 text-slate-700 border-slate-200', dot: 'bg-slate-700', icon: Truck },
} as const
    ...
    <div className="hidden md:block w-full overflow-x-hidden px-4 pb-2 md:px-0">
      <div className="space-y-3">
            ...
            {quickTiles.map((tile) => {
              ...
              return (
                <button
                  key={tile.id}
                  type="button"
                  onClick={() => onQuickTileClick(tile.path)}
                  className={cn(
        ...
        <section className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button type="button" onClick={onReportClick} className="min-w-0 rounded-2xl border border-red-200 bg-red-50 p-4 text-left shadow-sm transition active:scale-[0.99]">
            ...
            <button type="button" onClick={onReportClick} className="min-w-0 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-left shadow-sm transition active:scale-[0.99]">
            ...
            <button type="button" onClick={onReportClick} className="min-w-0 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-left shadow-sm transition active:scale-[0.99]">
            ...
            <button type="button" onClick={onReportClick} className="min-w-0 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left shadow-sm transition active:scale-[0.99]">
        ...
        <section className="pt-1">
          <Button variant="outline" className="w-full rounded-2xl border-zinc-200 bg-card shadow-sm" onClick={onOpenQuickAccess}>
            More <ChevronDown className="ml-2 h-4 w-4" />
          </Button>

          <Sheet open={quickAccessOpen} onOpenChange={onQuickAccessOpenChange}>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-2">
                      {recentProjects.map((project) => (
                        <button key={project.id} type="button" onClick={() => onProjectSelect(project)} className="flex w-full items-center justify-between rounded-2xl border border-border bg-card px-4 py-3 text-left transition hover:bg-muted/50">
