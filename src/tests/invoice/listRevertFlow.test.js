...
test('invoice list quote action routes into the invoice detail revert flow', () => {
  ...
  assert.match(viewInvoiceSource, /const openRevertFlow = useCallback\(\(\) =>/)
  assert.match(viewInvoiceSource, /requestAnimationFrame\(\(\) => \{\s*ui\.openModal\(MODAL_REVERT\)/)
  assert.match(viewInvoiceSource, /openRevertFlow\(\)/)
  assert.match(viewInvoiceSource, /onRevert=\{openRevertFlow\}/)
