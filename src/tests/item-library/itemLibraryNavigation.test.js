...
const itemHistoryRowPath = path.resolve('src/modules/item-library/components/ItemHistoryRow.tsx')
...
test('item library is routed and exposed through app navigation', () => {
  ...
  const historyRowSource = fs.readFileSync(itemHistoryRowPath, 'utf8')
  ...
  assert.match(detailPanelSource, /Price Intelligence/)
  assert.match(historyRowSource, /getHistoryDocumentHref/)
})
